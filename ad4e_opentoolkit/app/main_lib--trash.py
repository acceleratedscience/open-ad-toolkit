...
# Core
from ad4e_opentoolkit.core.lang_file_system import (
    import_file,
    export_file,
    copy_file,
    remove_file,
...
from ad4e_opentoolkit.core.lang_workspaces import (
    create_workspace,
    remove_workspace,
    list_workspaces,
    set_workspace,
...
from ad4e_opentoolkit.app.global_var_lib import (
    _meta_registry_session as _meta_registry_session,
)
from ad4e_opentoolkit.app.global_var_lib import (
    _meta_login_registry as _meta_login_registry,
)
...
# Helpers
from ad4e_opentoolkit.helpers.output import (
    msg,
    output_text,
    output_error,
    output_warning,
...
from ad4e_opentoolkit.helpers.general import (
    refresh_prompt,
    user_input,
    validate_file_path,
    ensure_file_path,
...
def list_toolkits(cmd_pointer, parser):
    ...
    return output_table(
        ...
        note=msg("all_toolkits_currently_installed"),
    )
    ...
    if toolkit_name.upper() not in cmd_pointer.settings["toolkits"]:
        ...
        if load_ok:
            ...
            try:
                ...
                # Error logging in.
                output_error(
                    msg("err_login", toolkit_name, err, split=True),
    ...
    try:
        if filename.split(".")[-1].lower() == "csv":
            ...
            try:
                ...
                return output_error(
                    msg("fail_file_doesnt_exist", file_path), cmd_pointer
                )
            except BaseException as err:
            ...
            return output_error(
                msg("invalid_file_format", "csv", split=True), cmd_pointer
            )
...
def display_data__save(cmd_pointer, parser):
    ...
    file_path = (
        parser["file_path"] if "file_path" in parser else None
    )  # Parser as_dict?
...
def edit_config(cmd_pointer, parser):
    ...
    if "schema" in parser.as_dict():
        ...
        schema = (
            workspace_path
            + re.sub(r"\.json$", "", parser.as_dict()["json_file"])
            + "-schema.json"
        )
    ...
    if os.path.isfile(file_to_edit):
        ...
        return output_error(
            msg("fail_file_doesnt_exist", parser.as_dict()["json_file"]), cmd_pointer
        )
