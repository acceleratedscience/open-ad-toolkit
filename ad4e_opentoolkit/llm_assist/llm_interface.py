...
import shutil
import glob
from ad4e_opentoolkit.llm_assist.prime_chat import Chatobject
from ad4e_opentoolkit.helpers.output        import  output_text, output_error
from ad4e_opentoolkit.helpers.output        import output_warning, output_success
from ad4e_opentoolkit.app.global_var_lib    import _repo_dir
from ad4e_opentoolkit.app.global_var_lib    import _meta_dir
from ad4e_opentoolkit.helpers.credentials   import load_credentials
from ad4e_opentoolkit.helpers.credentials   import write_credentials,get_credentials
...
def create_train_repo(included_sources=DEFAULT_SOURCES_LIST, location_for_documents=PROMPT_DIR, document_types=STANDARD_FILE_TYPES_EMBED):
    ...
    # catch any error that may be caused by OS in creating document localtion
        ...
        output_error(f"unable to create training repository directory {location_for_documents}"+str(err),return_val=False)
        return False
...
def how_do_i(cmd_pointer, parser):
    ...
    if cmd_pointer.llm_handle is None or cmd_pointer.refresh_vector is True:
        ...
        try:
            cmd_pointer.llm_handle = Chatobject(API_key=get_api_key(llm_name=cmd_pointer.llm_service, cmd_pointer=cmd_pointer)['auth']['api_key'],
                                                 ...
                                                 document_types=EXTENDED_FILE_TYPES_EMBED, refresh_vector=cmd_pointer.refresh_vector,
                                                 llm_service=cmd_pointer.llm_service, llm_model=cmd_pointer.llm_model)
...
def set_llm(cmd_pointer, parser):
    ...
    if llm_name.upper() in SUPPORTED_LLMS:
        ...
        return output_success(' The Following has been set as the current llm service ' + llm_name.upper(), cmd_pointer, pad=1)
...
def get_api_key(llm_name, cmd_pointer):
    ...
    if api_config is None:
        ...
        api_config =get_credentials(cmd_pointer=cmd_pointer, credentials=api_config,creds_to_set=['host','auth:api_key'])
