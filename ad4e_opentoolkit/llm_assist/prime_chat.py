...
from ad4e_opentoolkit.helpers.output    import output_error, output_warning
...
class  Chatobject( ):
    ...
    def __init__(self,target='OPENAPI',organisation='org-V3VSRAXasFUnufPII8o1DIIk'
                 ,API_key=None,vector_db='FAISS',document_folders=["./"],
                 document_types=document_types,db_dir_override=None,refresh_vector=False,
                 llm_model='gpt-4',llm_service='OPENAI'):
        ...
        if self.vector_db == 'FAISS':
            try:
                ...
                raise Exception(f"the vector db {self.db_handle} Was not able to be loaded") from e #pylint: disable=broad-exception-raised
        else:
            raise Exception(f"the vector db {self.db_handle} is not currently supported") #pylint: disable=broad-exception-raised
    ...
    def load_faiss_db(self,refresh=True):
        ...
        if self.llm_service=='OPENAI' :
            try:
                ...
                raise Exception("Error: cannot initialise embeddings, check API Key") from e #pylint: disable=broad-exception-raised
        elif self.llm_service=='WATSONX':
            try:
                embeddings = MosaicMLInstructorEmbeddings(endpoint_url=self.organisation ,
                                                          mosaicml_api_token=self.API_key)
            except Exception as e:
                raise Exception("Error: cannot initialise embeddings, check API Key") from e #pylint: disable=broad-exception-raised
        ...
        if refresh is not True :
            try:
                if self.vector_db == 'FAISS':
                    main_db=FAISS.load_local(os.path.expanduser(self.db_dir+"/faiss_index"), embeddings) #pylint: disable=no-member
                return main_db
