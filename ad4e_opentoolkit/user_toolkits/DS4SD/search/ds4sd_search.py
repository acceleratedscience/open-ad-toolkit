...
def search(inputs: dict,cmd_pointer):
    ...
    result=[ {   "Domains":  '/ '.join(c.metadata.domain),
    ...
    if val_index_key not in index_list and val_index_key not in index_name_list:
        output_error('Invalid collection key or name, please choose from the following: ',cmd_pointer=cmd_pointer,return_val=False)
        collectives = pd.DataFrame(result)
    ...
    if cmd_pointer.notebook_mode is True:
        ...
        if 'save_as' in inputs:
            ...
            if 'Snippet' in cmd_line_result:
                cmd_line_result['Snippet'] = cmd_line_result['Snippet'].apply(lambda x: style(x))#pylint diable:unnecessary-lambda
