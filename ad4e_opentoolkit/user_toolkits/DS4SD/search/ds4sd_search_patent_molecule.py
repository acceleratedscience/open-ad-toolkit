...
def search_patent_molecule(inputs: dict,cmd_pointer):
    ...
    try:
        if valid_smiles(inputs['smiles']) is True:
            query= PatentsWithMoleculesQuery(molecules=[MolId(type=MolIdType.SMILES,value=inputs['smiles'])], num_items=20,)
            result_type='SMILES'
        elif valid_inchi(inputs['smiles']) is True:   
            query= PatentsWithMoleculesQuery(molecules=[MolId(type=MolIdType.INCHI,value=inputs['smiles'])], num_items=20,)
            result_type='Inchi'
        else:
