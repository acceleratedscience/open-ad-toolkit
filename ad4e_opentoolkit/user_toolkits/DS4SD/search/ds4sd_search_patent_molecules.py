...
def search_patent_molecules(inputs: dict, cmd_pointer):
    ...
    if isinstance(inputs['from_source'],dict) and inputs['from_source']['from_list'] is not None:
        ...
        try:
            ...
            output_error("Could not load valid list from dataframe column column 'PATENT ID' or 'patent id' "+str(err), cmd_pointer=cmd_pointer, return_val=False)
            return True
        ...
        try:
            ...
            output_error("Could not load valid list from file column 'PATENT ID' or 'patent id' ", cmd_pointer=cmd_pointer, return_val=False)
            return True
    try:
