...
def search_similar_molecules(inputs: dict,cmd_pointer):
    ...
    if 'save_as' in inputs:
        ...
        output_text(" <green> File successfully saved to workspace. </green>",cmd_pointer=cmd_pointer,return_val=False)
    output_text(" ",cmd_pointer=cmd_pointer, return_val=False )
    output_text("<h2>  Similarity Search Results for smiles molecule: </h2>  ",cmd_pointer=cmd_pointer, return_val=False )
