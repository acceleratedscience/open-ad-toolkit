...
def get_reaction_from_smiles(reaction_smiles: str) -> Chem.rdChemReactions.ChemicalReaction: # pylint: disable=no-member
   ...
   # getting our input source for the reactions
    ...
    if isinstance(inputs['from_source'],dict) and inputs['from_source']['from_list'] is not None:
        try:
            ...
            output_error("unexpected pyparsing error. Please screenshot and report circumstance to OpenAD team", cmd_pointer=cmd_pointer, return_val=False)
        ...
        try:
            ...
            if from_list == []:
                raise Exception("No Provided reactions, data frame should have column 'reactions'" ) #pylint: disable=broad-exception-raised
        except Exception: #pylint: disable=broad-exception-caught
            output_error("Could not load valid list from dataframe column 'reactions' ", cmd_pointer=cmd_pointer, return_val=False)
            return False
        ...
        try:
            ...
            output_error("Could not load valid list from file column 'reactions' ", cmd_pointer=cmd_pointer, return_val=False)
            return False
    ...
    for entry in from_list:
        ...
        if len(error_list)>0:
            ...
            output_warning("info: This rection will be skipped: "+entry+" ", cmd_pointer=cmd_pointer, return_val=False)
            continue    
    ...
    for reaction_predictions in cached_results:
        ...
        output_text(f"<success> Saved Reaction results for:</success> {cached_cannonical_list[reaction_no]}", cmd_pointer=cmd_pointer, return_val=False)
        ...
        for j, prediction in enumerate(reaction_predictions["results"], 1):
            ...
            output_text(f'<success>         Product(s) </success>{j} {product_smiles}, With confidence {confidence}', cmd_pointer=cmd_pointer, return_val=False)
        output_text("\n", cmd_pointer=cmd_pointer, return_val=False)
    ...
    if len(new_from_list) > 0:
        ...
        for i, reaction_predictions in enumerate(x['predictions'], 1):
            output_text('\n', cmd_pointer=cmd_pointer, return_val=False)
            output_text(f' <success> Outcomes for Reaction: </success>   {new_cannonical_list[reaction_no]}:', cmd_pointer=cmd_pointer, return_val=False) 
