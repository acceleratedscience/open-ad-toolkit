...
def collect_reactions_from_retrosynthesis(tree: Dict)  -> List[str] :
    ...
    if 'children' in tree and len(tree['children']):
        reactions.append( AllChem.ReactionFromSmarts('{}>>{}'.format(  '.'.join([node['smiles'] for node in tree['children']]),tree['smiles']), useSmiles=True) ) # pylint: disable=no-member
...
def predict_retro(inputs: dict, cmd_pointer):
    ...
    try:
        ...
        for index, tree in enumerate(predict_automatic_retrosynthesis_results['retrosynthetic_paths']):
            ...
            output_text('<h2> <success>Showing path </success> {} <success> with confidence </success>{}:'.format(index, tree['confidence'])+"</h2>", cmd_pointer=cmd_pointer, return_val=False)
            for reaction in collect_reactions_from_retrosynthesis(tree):
                output_text('<success> Reaction: </success>'+reactions_text[i], cmd_pointer=cmd_pointer, return_val=False)
                if cmd_pointer.notebook_mode is True:
