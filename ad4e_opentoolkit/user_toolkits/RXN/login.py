...
def login(cmd_pointer):
    ...
    if 'RXN' not in cmd_pointer.login_settings['toolkits']:
        ...
        try:
            ...
            output_error(msg('err_login', 'RXN',f"system error {e}", split=True), cmd_pointer=cmd_pointer, return_val=False)
            return False, None
...
# if no Authentication file ask for authentication details and create
    ...
    try:
        ...
        output_error(msg('err_login', 'RXN',f"Unable to connect to  {config_file['host']}", split=True), cmd_pointer=cmd_pointer, return_val=False)
        output_error(msg('err_login', 'RXN',f"system error {e}", split=True), cmd_pointer=cmd_pointer, return_val=False)
        return False, None
...
def get_creds(cred_file,cmd_pointer):
    ...
    if api_config is None:
        ...
        output_text(f"Enter the Hostname: if the hostname is left blank it will default to '{DEFAULT_URL}' ", cmd_pointer=cmd_pointer, return_val=False)
