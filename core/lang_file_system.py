...
# Helpers
from helpers.general import confirm_prompt
...
from global_var_lib import _date_format as _date_format
...
def list_files(cmd_pointer, parser):
    ...
    from helpers.style_parser import parse_tags
    ...
    files_data = [
        (
            ...
            os.stat(os.path.join(path, f)).st_atime
        )
        for f in files_list
    ]
...
def copy_file(cmd_pointer, parser):
    ...
    elif parser['destination'].upper() != source_workspace_name and dest_workspace_name not in cmd_pointer.settings['workspaces']:
        # Invalid destination
