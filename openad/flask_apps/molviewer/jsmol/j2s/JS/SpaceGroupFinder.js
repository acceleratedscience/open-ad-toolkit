...
if (xyzList.indexOf (".") < 0) xyzList += ".1";
var sgdata = uc.getSpaceGroupJSON (vwr, "ITA", xyzList, 0);
...
if (isKnown) {
var sgjmol = JS.SpaceGroup.determineSpaceGroupNA (xyzList, null);
sg.intlTableNumberFull = (sgjmol == null ? name : sgjmol.intlTableNumberFull);
}var u = sgdata.get ("u");
var tr = sgdata.get ("tr");
sg.intlTableNumberExt = JU.PT.rep (u, " ", "") + ";" + sgdata.get ("sg") + "(" + tr + ")";
var axis = u.toLowerCase ().charAt (0);
if (JS.SpaceGroupFinder.isHexagonal (JU.PT.parseInt (sg.intlTableNumber), null) && axis != 'r') axis = 'h';
switch (axis) {
case 'a':
case 'b':
case 'c':
...
if (n <= 2 || isHexGroup && (sg.axisChoice == 'r' ? JS.SpaceGroupFinder.isRhombohedralCell (params) : JS.SpaceGroupFinder.isHexagonal (-1, params))) {
} else if (isHexGroup) {
if (sg.axisChoice == 'r') params =  Clazz.newFloatArray (-1, [params[0], params[0], params[0], 100, 100, 100]);
 else params =  Clazz.newFloatArray (-1, [params[0], params[0], params[2], 90, 90, 120]);
} else if (n >= 195) {
...
c$.isRhombohedralCell = Clazz.defineMethod (c$, "isRhombohedralCell", 
 function (params) {
return (JS.SpaceGroupFinder.approx0 (params[0] - params[1]) && JS.SpaceGroupFinder.approx0 (params[1] - params[2]) && params[3] != 90 && JS.SpaceGroupFinder.approx0 (params[3] - params[4]) && JS.SpaceGroupFinder.approx0 (params[4] - params[5]));
}, "~A");
...
var isRhombo = false;
var isCubic = false;
var absame = JS.SpaceGroupFinder.approx0 (params[0] - params[1]);
var bcsame = JS.SpaceGroupFinder.approx0 (params[1] - params[2]);
if (params[3] == 90) {
if (params[4] == 90) {
if (absame && params[0] != params[1]) System.out.println ("OHOH");
isTri = (absame && JS.SpaceGroupFinder.approx000 (params[5] - 120));
if (params[5] == 90) {
...
switch (abc) {
case 1:
if (JS.SpaceGroupFinder.approx0 (f = this.pt.x) || !JS.SpaceGroupFinder.approx0 (this.pt.y) || !JS.SpaceGroupFinder.approx0 (this.pt.z)) continue;
break;
case 2:
if (JS.SpaceGroupFinder.approx0 (f = this.pt.y) || !JS.SpaceGroupFinder.approx0 (this.pt.x) || !JS.SpaceGroupFinder.approx0 (this.pt.z)) continue;
break;
default:
case 3:
if (JS.SpaceGroupFinder.approx0 (f = this.pt.z) || !JS.SpaceGroupFinder.approx0 (this.pt.x) || !JS.SpaceGroupFinder.approx0 (this.pt.y)) continue;
break;
}
var n = JS.SpaceGroupFinder.approxInt (1 / f);
...
switch (abc) {
case 1:
f = JS.SpaceGroupFinder.approxInt (n * this.atoms[i].x);
break;
case 2:
f = JS.SpaceGroupFinder.approxInt (n * this.atoms[i].y);
break;
case 3:
f = JS.SpaceGroupFinder.approxInt (n * this.atoms[i].z);
break;
}
if (f != 0) {
this.atoms[i] = null;
...
c$.approx000 = Clazz.defineMethod (c$, "approx000", 
 function (f) {
return (Math.abs (f) < 1.0E-4);
}, "~N");
c$.approx0014 = Clazz.defineMethod (c$, "approx0014", 
 function (f) {
return (Math.abs (f) < JS.SpaceGroupFinder.SLOP0014);
}, "~N");
c$.approxInt = Clazz.defineMethod (c$, "approxInt", 
 function (finv) {
...
Clazz.defineMethod (c$, "latticeShift", 
 function (a, b) {
var is1 = (JS.SpaceGroupFinder.approx0014 (Math.abs (a.x - b.x) - 1) || JS.SpaceGroupFinder.approx0014 (Math.abs (a.y - b.y) - 1) || JS.SpaceGroupFinder.approx0014 (Math.abs (a.z - b.z) - 1));
if (is1) {
}return is1;
}, "JU.P3,JU.P3");
