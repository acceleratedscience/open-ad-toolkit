...
class MoleculesApi:
    ...
    def query_molset(self):
        ...
        return create_molset_response(molset, cache_id, query), 200
...
def smiles_file_to_molset(path_absolute):
    """
...
def create_molset_response(molset, cache_id, query={}):
    ...
    # Parse the query
    search_str = unquote(query["search"]) if "search" in query else ""
    smarts_mode = query["smartsMode"] if "smartsMode" in query else False
    page = int(query["page"]) if "page" in query else 1
    sort = query["sort"] if "sort" in query else None
    page_size = 48  # Hardcoded for now
    ...
    try:
        if (sort) == "name":
            results = sorted(results, key=lambda mol: _sort_mol(mol, "identifiers", sort))
        elif sort:
            results = sorted(results, key=lambda mol: _sort_mol(mol, "properties", sort))
    except TypeError as err:
        # In the edge case where our dataset mixes string and
        # number values, we want to avoid crashing the app.
        output_error(err)
...
def _sort_mol(mol, category, sort_key):
    """
    ...
    mol: dict
        A molecule object.
    category: str
        The category where to look for the sort key,
        either "identifiers" or "properties".
    sort_key: str
        The key of the category whose value we'll sort by.
        Eg. 'name' (identifier) or 'molecular_weight' (property).
    """
    value = (mol.get(category) or {}).get(sort_key)
