...
import json
import logging
from core.grammar import statement_builder
from helpers.output import msg, output_error

# Globals
from global_var_lib import _meta_dir_toolkits
from global_var_lib import _meta_workspaces
...
def execute_tookit(cmd_pointer, parser):
    ...
    logging.basicConfig(level=logging.INFO, filename=_meta_workspaces + "/workspace.log", format="%(asctime)s - %(message)s")

    for i in glob.glob(_meta_dir_toolkits + "/" + cmd_pointer.toolkit_current.toolkit_name + "/**/" +
                       cmd_pointer.toolkit_current.methods_library[index] + ".py", recursive=True):
